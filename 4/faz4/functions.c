...
void get_n ( FILE* , int* , int* ) ; //get n & num  ;
void get_i_co ( FILE* , int , int , char [][25] ) ; //get p_names & cordinates ;
...
void get_n( FILE* file , int* n , int* num )
{
    fscanf (file,"%d",n) ;
    fscanf (file,"%d",num) ;
    return ;
...
void get_i_co ( FILE* file , int n , int num , char p_name [][25] )
{
    int x , y , a ;
    char c ;
    for ( int k = 0 ; k < 2 ; k ++ )
